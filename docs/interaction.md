...
先丢弃脏区再退出。字段说明固定在底栏槽位，焦点移动不改变列表行高。鼠标：
字段行点击 = 设焦点并执行该行 Enter 动作，悬停即移动焦点（键导航后短暂忽略
悬停），滚轮走焦点。dsh-tui 自身命名空间的字段写入 settings.yaml 用户层并
**实时生效**（`lang`、`statusBar.*` 等）；未声明 TUI 区块的命名空间收在
「其他」分类下只读列出，需手工编辑 `~/.dsh/settings.yaml`。
