...
`/settings` 打开插件设置编辑器，顶部分类页（常规 / 底栏 / 快捷键）切换时
标题与底栏保持不动。编辑是**暂存制**：`←`/`→` 换分类、`↑`/`↓` 移动字段、
`Enter` 就地开关布尔或打开就地选择/文本编辑，`s` 保存 / `d` 放弃 / `Esc`
先丢弃脏区再退出。字段说明固定在底栏槽位，焦点移动不改变列表行高。审批 /
问卷打断会暂时卸下设置屏，但焦点、分类与未保存草稿会保留到打断结束（显式
关闭才清空）。鼠标：字段行点击 = 设焦点并执行该行 Enter 动作，悬停即移动焦点
（键导航后短暂忽略悬停），滚轮走焦点。dsh-tui 自身命名空间的字段写入
settings.yaml 用户层并**实时生效**（`lang`、`statusBar.*` 等）；未声明 TUI
区块的命名空间收在「其他」分类下只读列出，需手工编辑 `~/.dsh/settings.yaml`。
...
| 单击 /settings 字段行 | 设焦点并执行该行的 Enter 动作（boolean 就地开关、select 打开就地选择、文本进编辑态）；悬停即移动焦点（lazygit 语义，键导航后短暂锁定）；编辑/选择态整屏不响应防误触 |
