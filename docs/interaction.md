...
`/settings` 打开插件设置编辑器，卡片式分区 + 顶部分类页（常规 / 底栏 / 快捷键），切换分类时
标题与底栏保持不动。**自动保存**：`←`/`→` 换分类、`↑`/`↓` 移动字段、`Enter` 就地开关布尔
（`[✓]` chip）或打开就地选择/文本编辑（确认即写盘）、`Esc` 退出（分组 Esc 返回上级，编辑态
Esc 取消草稿）。字段说明固定在底栏 help bar，焦点移动不改变列表行高；保存成功/失败在底栏
通知槽短暂提示。鼠标：字段行点击 = 设焦点并执行该行 Enter 动作，悬停即移动焦点（键导航后
短暂忽略悬停），滚轮走焦点。dsh-tui 自身命名空间的字段写入 settings.yaml 用户层并**即时生效**
（`lang`、`statusBar.*` 等）；未注册 TUI 区块的命名空间不在屏内列出，需手工编辑
`~/.dsh/settings.yaml`。
