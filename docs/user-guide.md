...
| `/preset` | `<id>` / `status` | Agent 预设切换：官方 `standard` / `ptc` / `minimal` / `cordis` + TUI 打包**梁神模式 `liangshen`** + 用户自定义；**已开始的会话不可切换**（blank-only 锁定）。持久化 `~/.dsh-tui/agent-preset.json` |
...
| Agent 预设 | `/preset` | `standard` / `ptc` / `minimal` / `cordis` + **梁神模式 `liangshen`**；**已开始会话不可切换**（blank-only） |
